...
describe("auth roles and permissions", () => {
  ...
  it("does not implicitly bootstrap the first authenticated user as admin", () => {
    ...
    const bootstrapConfig: LegitConfig = {
      ...config,
  ...
  it("allows explicit bootstrap admin emails from the environment", () => {
    const handle = createSqliteDatabase(":memory:");
    const original = process.env.LEGIT_BOOTSTRAP_ADMIN_EMAILS;

    try {
      process.env.LEGIT_BOOTSTRAP_ADMIN_EMAILS = "owner@example.com, second@example.com";
      ...
      if (original === undefined) {
        delete process.env.LEGIT_BOOTSTRAP_ADMIN_EMAILS;
      } else {
        process.env.LEGIT_BOOTSTRAP_ADMIN_EMAILS = original;
      }
      handle.sqlite.close();
    }
  });
