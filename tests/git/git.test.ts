...
describe("git helpers", () => {
  it("builds transient askpass auth env and redacts credentialed URLs", () => {
    const env = buildGitAuthEnv(repo, { LEGIT_GITHUB_TOKEN: "secret-token" });

    expect(env.LEGIT_GIT_USERNAME).toBe("x-access-token");
    expect(env.LEGIT_GIT_PASSWORD).toBe("secret-token");
    expect(env.GIT_ASKPASS).toMatch(/legit-git-askpass/);
  ...
  it("rejects repo path traversal", () => {
    const root = path.join(os.tmpdir(), "legit-repos-test");
  ...
  it("runs clone for missing repositories and pull for existing repositories", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "legit-git-"));
    ...
    await cloneOrPullRepository(repo, {
      reposRoot: root,
      env: { LEGIT_GITHUB_TOKEN: "secret-token" },
      runner,
    });
    ...
    expect(calls[0].env?.LEGIT_GIT_PASSWORD).toBe("secret-token");
  ...
  it("does not persist service tokens in the cached repository remote config", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "legit-git-config-"));
  ...
  it("runs branch workflow with service credentials and returns URLs", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "legit-commit-"));
    ...
    expect(calls[1]).toEqual(["checkout", "-B", expect.stringMatching(/^legit\/edit-index-/), "origin/main"]);
    expect(calls).toContainEqual(["push", "-u", "origin", expect.stringMatching(/^legit\/edit-index-/)]);
    ...
    expect(result).toEqual({
      ...
      mode: "merge-request",
      branch: expect.stringMatching(/^legit\/edit-index-/),
      branchUrl: expect.stringMatching(/^https:\/\/github.test\/tree\/legit\/edit-index-/),
  ...
  it("creates GitHub pull requests through the provider API without exposing tokens", async () => {
    ...
    const provider = createGitHubProvider({
      env: { LEGIT_GITHUB_TOKEN: "secret-token" } as unknown as NodeJS.ProcessEnv,
    ...
    const result = await provider.createMergeRequest(repo, {
      sourceBranch: "legit/edit-index-abc",
      targetBranch: "main",
      title: "Update docs",
  ...
  it("creates GitLab merge requests through the provider API", async () => {
    ...
    const provider = createGitLabProvider({
      env: { LEGIT_GITLAB_TOKEN: "secret-token" } as unknown as NodeJS.ProcessEnv,
      fetch: async (url, init) => {
        calls.push({ url: String(url), init: init ?? {} });
  ...
  it("classifies provider API failures in merge-request mode", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "legit-provider-fail-"));
    fs.mkdirSync(path.join(root, repo.id, "docs"), { recursive: true });
    fs.writeFileSync(path.join(root, repo.id, "docs", "index.md"), "# Commit", "utf8");
