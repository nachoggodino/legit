...
export const DEFAULT_DATABASE_PATH = "/data/legit.db";
...
function isProductionBuild(): boolean {
  return process.env.LEGIT_BUILD_PHASE === "1" || process.env.NEXT_PHASE === "phase-production-build";
}

export function resolveDatabasePath(): string {
  if (process.env.LEGIT_DATABASE_PATH) {
    return process.env.LEGIT_DATABASE_PATH;
  }

  if (isProductionBuild()) {
    return path.join(os.tmpdir(), "legit-build.db");
  }
