...
export function resolveRepoPath(repoId: string, reposRoot = process.env.LEGIT_REPOS_ROOT ?? DEFAULT_REPOS_ROOT): string {
...
export function buildGitAuthEnv(
  ...
  return {
    ...
    LEGIT_GIT_USERNAME: authConfig.username,
    LEGIT_GIT_PASSWORD: token,
  };
}
...
function getAskPassScriptPath(): string {
  ...
  const scriptPath = path.join(os.tmpdir(), `legit-git-askpass-${process.pid}.sh`);
  fs.writeFileSync(
    scriptPath,
    [
      "#!/bin/sh",
