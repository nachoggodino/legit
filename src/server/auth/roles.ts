...
export function assignRoleForEmail(email: string, config: LegitConfig): Role {
...
export function hasConfiguredAdmins(config: LegitConfig): boolean {
...
export function getBootstrapAdminEmails(env = process.env): Set<string> {
  return new Set(
    (env.LEGIT_BOOTSTRAP_ADMIN_EMAILS ?? "")
      .split(",")
      .map(normalizeEmail)
      .filter(Boolean),
  );
