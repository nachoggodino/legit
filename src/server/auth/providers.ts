...
const placeholderValuePattern = /(replace-with|your-[a-z0-9-]+|example)/i;
...
function hasEnv(...names: string[]): boolean {
  return names.every(hasConfiguredEnvValue);
}
...
export function buildAuthProviderStatuses() {
  return providerRequirements.map((provider) => {
    const missingEnv = provider.env.filter((name) => !hasConfiguredEnvValue(name));
