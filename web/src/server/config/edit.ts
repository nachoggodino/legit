...
import { copisaurusConfigSchema, repositoryConfigSchema, type CopisaurusConfig } from "./schema";
...
export function updateSafeRepositoryConfig(input: unknown, filePath = resolveConfigPath()) {
  ...
  const validated = copisaurusConfigSchema.parse(next);
